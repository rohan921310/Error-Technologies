...
class Admin_model extends CI_Model
{
    ...
    function accept_bid()
    {
        ...
        $this->db->select('*');
        $this->db->from('advertisement');
        $this->db->where('advertisement_id', $add_id);
        ...
        if($login1){
            ...
            $query2 = $this->db->get();
            $login2 = $query2->row();
            $new = $login2->user_wallet_amount + $login1->product_offer_price; 
            // echo( $login2->user_details_id);
            // die;
            $formArray1 = array(
                'user_wallet_amount' => $new
            );
            $this->db->where('user_details_id', $login2->user_details_id);
    ...
    function all_bids1()
    {
        ...
        $this->db->from('all_bids');
        $this->db->where('all_bids.tasker_details_id', $tasker_details_id);
        $this->db->where('all_bids.bid_status', 0);
        $this->db->join('advertisement', 'advertisement.advertisement_id  = all_bids.add_id', 'left');
        $this->db->join('user_details', 'user_details.user_details_id  = all_bids.user_details_id', 'left');
        $this->db->limit(6);
        $this->db->order_by('all_bids_id', 'DESC');
    ...
    function fetch_active_projects()
    {
        ...
        $this->db->join('user_details', 'user_details.user_details_id  = payments_history.user_details_id', 'left');
        // $this->db->join('advertisement', 'advertisement.advertisement_id  = all_bids.add_id', 'left');
        $this->db->order_by('payments_history_id', 'ASC');
    ...
    function tasker_do_payment()
    {
        ...
        $payments_history_id =  $this->input->get('all_bids_id');
        $this->db->select('*');
		$this->db->from('payments_history');
		$this->db->where('payments_history_id',$payments_history_id);
        ...
        $this->db->where('payments_history_id', $payments_history_id);
        $this->db->update('payments_history', $formArray);
		...
		if($login){
            $new = $login->payouts + $login1->req_wallet_amount ;
            $formArray1 = array(
                'payouts' => $new,
            );
            $this->db->where('tasker_details_id', $tasker_details_id);
            $this->db->update('tasker_details', $formArray1);
			$this->session->set_userdata('payouts',$login->payouts);
    ...
    function min_wid_amount()
    {
        $min_wid_amount =  $this->input->post('min_wid_amount');
        $tasker_details_id = $this->session->userdata('tasker_details_id');
        $formArray = array(
            'min_wid_amount' => $min_wid_amount,
        );
        $this->db->where('tasker_details_id', $tasker_details_id);
        $this->db->update('tasker_details', $formArray);
			$this->session->set_userdata('min_wid_amount',$min_wid_amount);

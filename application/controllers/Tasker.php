...
class Tasker extends CI_Controller
{
	...
	function tasker_dashboard()
	{
		...
		$data['all_bids'] = $this->Admin_model->all_bids1();
	...
	{
		$this->Admin_model->min_wid_amount();
		$this->session->set_flashdata('msg', 'Minimum Amount Updated');
		$this->session->set_flashdata('msg_class', 'bg-success text-white');
		redirect(base_url() . 'payments_tasker');
	}
}
